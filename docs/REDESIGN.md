...
| 3 | **Events** | **RESKIN** of `ui_events:2597-2673`. Adds: config column, `WITHIN` field, Delivery segmented, stats card, kind badges, accent flash-in. | `EventMonitor` ✔ · **delivery rate** = computable client-side ✔ · **"Sink queue"** ✖ (not observable — replace with real mpsc backlog or drop) · **Permanent delivery** ✖ (would write `__EventFilter`/consumer/binding) · **remote monitoring** ✖ (`monitor.rs:87` is local-only) | **done** (4.8–4.13). Two further things the mock claims and the data does not carry: the event's `__CLASS` and its `TargetInstance`'s class (see 4.11). Remote monitoring is still ✖ and is now *stated on screen* — the column warns that the subscription is local whenever the app is connected elsewhere, because a monitor silently watching the wrong machine is the worst failure available to this view. |
...
| 5.14b | **The count was seven, not two, and one of them executed a method** | Measured in Phase 5 by putting a worker into alt-cred mode with credentials that cannot connect and firing every request shape at it (`examples/altcred.rs`, and the same harness against the pre-fix tree). Answering from a current-user connection: `ClassSchema`, `ClassMof`, `ListInstances`, `BuildSearchIndex`, `NetworkSnapshot` (empty, per 5.14), `ListEventSubscriptions` (empty) — **and `InvokeMethod`, which ran `StdRegProv.EnumKey` on the local machine and returned `ReturnValue=0`** while the caller believed it was invoking on a remote host under other credentials. A read that answers about the wrong computer is bad; a *state-changing call* that lands on the wrong computer is the worst thing in this codebase. Also found: `ListEventSubscriptions` returned `Ok` with an empty report when it could not read a single namespace — "no persistence found" and "we could not look" were the same answer. | **R** |
...
| **Impersonation level dropdown** | **Real, and on both paths.** `remote.rs` hand-calls `CoSetProxyBlanket`, so `RPC_C_IMP_LEVEL_{IDENTIFY,IMPERSONATE,DELEGATE}` is one parameter away. ~~on the alt-cred path only — on the SSO path we go through `wmi::WMIConnection`, which doesn't expose it~~ — **wrong since Phase 3**: the SSO path is `enumerate::DirectConn`, which sets its own blanket, and task 5.10 wired the level through both. Measured locally at `Identify`: the connect probe and a query are refused (`WBEM_E_ACCESS_DENIED`), `StdRegProv.EnumKey` is refused (`WBEM_E_PROVIDER_NOT_CAPABLE`), a schema reflection still succeeds. So the control can be **enabled always**, and it is not decorative — but `Identify` breaks nearly everything, which is the truth about the setting rather than a reason to hide it. |
...
| **Status bar "WmiPrvSE 0.6% · 41 MB"** | **Real** (§5.11) but costs a WMI query every N s against the local box. Ship **opt-in** (Settings → "Show provider host stats", default off) rendering `WmiPrvSE — · — MB` when off. Never fake. **Measured in Phase 5:** the whole `ListProviders` round trip — provider rows, image names, per-namespace `HostingModel`, the perf join, the quota and the CPU count — is 730–1017 ms, so "every N s" needs N in seconds and not fractions of one. And there is no single `WmiPrvSE`: four host processes were live here, so the status bar has to name *which* one, or sum them and say so. |
| **Events "Sink queue" stat** | **Not observable** in a polling `IWbemObjectSink` design. **Replace** with the mpsc backlog we actually own (`MonitorMsg` depth) labelled "Queued", or drop the tile. **Shipped as "Queued" in Phase 4**, as `depth now · peak since Start` — the instantaneous read is 0 most frames even while a burst is being worked through (19 with a peak of 111, measured), so the peak is what makes the tile worth having. Its tooltip says outright that this is not WMI's sink queue. |
| **Events "Provider" stat** | **Added to this table in Phase 4, and dropped from the card.** Nothing in a notification reply names the provider that produced the event, and the mock's "EventProv (WMI)" is a label, not a reading. Recovering it would mean querying `__Win32Provider`/`__EventProviderRegistration` — a new core request for a decorative line. The slot now carries **Delivery** (`polled / N s` or `pushed`), read off the query's event class, which is a fact we hold. |
...
| **"SELECT is projected server-side / ORDER BY is evaluated locally"** | ~~**True statement about WQL**, worth keeping~~ — **half of it is false.** *Projection* is real: `SELECT Name, ProcessId, ThreadCount, WorkingSetSize FROM Win32_Process` returns exactly those four columns, captured. *"ORDER BY is evaluated locally"* is **not true** — WQL's parser rejects every `ORDER BY` form with `WBEM_E_INVALID_QUERY` (0x80041017) before producing a row, measured across `Win32_Process`, `Win32_Service` and `Win32_OperatingSystem`, single- and multi-line, with and without `ASC`. Nothing sorts it client-side either; the table sorts on a header click. The derivation requirement was right and stands; the *sentence* now reads "ORDER BY is not valid WQL — sort by clicking a column". See `docs/FINDINGS.md`. |
...
*Status (Phase 5, tasks 5.6–5.9):* closed at the root rather than per call site. There is now one way to bind a namespace — `worker::bind`, returning `enumerate::Bound` — and `wmi::WMIConnection`, the only transport that cannot carry a credential, is gone from the worker. The mitigation banner (5.19) has nothing left to list. Note the remaining hole is the *opposite* of the old one: an operation that cannot work over alternate credentials now fails loudly instead of running as the wrong user, and none of it has been exercised against a live remote host.
...
- [x] **4.1** `[gui]` `views/query.rs`: WQL editor with a line-number gutter (`widgets::codeview`) + Run/Save/Export. *AC: gutter tracks wrapped lines correctly.* Note: `codeview::code_panel` is read-only, so the editor is a `TextEdit::multiline` with the gutter painted from its own `Galley` — the numbers come from `PlacedRow::ends_with_newline`, which is the only way to tell a wrapped continuation from a new line. `codeview::tint_line` **is** reused, for 4.3. Captured at 1 and 3 lines.
- [x] **4.2** `[gui]` Query status strip: "Completed in N ms · M rows · SELECT is projected server-side" from real `elapsed_ms`/`rows.len()`. *AC: the ms is the measured value, never a constant.* Captured at 79 ms / 1 row and 62 ms / 147 rows; the same query measured 65, 100, 54, 62 ms across four runs. `connect_ms` is reported apart from it, in the strip's tooltip.
- [x] **4.3** `[gui]` Query warn note ~~"ORDER BY is evaluated locally by the client"~~ shown **only** when a case-insensitive `ORDER BY` appears outside a string literal. *AC: a query without ORDER BY shows no note.* The trigger is as specified (a lexer scan, so `WHERE Name = 'order by'` does not fire it). **The sentence changed**: WQL rejects `ORDER BY` outright with `WBEM_E_INVALID_QUERY`, so the note reads "ORDER BY is not valid WQL — sort by clicking a column". Both branches captured. See `docs/FINDINGS.md`.
- [x] **4.4** `[gui]` Query result table on the kit + row-detail via the palette-style side reveal (replacing today's right `detail` panel). *AC: clicking a row shows its full property set.* Shipped as a second `Panel::right` present only while a row is selected. Captured with row 1 of 147 open.
- [x] **4.5** `[gui]` `Panel::right("vs_history").exact_size(262)`: query text, elapsed ms (colored warn when > 1000), row count, relative time, click-to-reload. *AC: clicking a history item reloads text + namespace.* Namespace is restored **first**, because `run_query` reads `active_ns` — the same ordering 4.16 turns on. A migrated v1 entry has no namespace and deliberately leaves the active one alone rather than guessing.
- [x] **4.6** `[gui]` `Config.history` entries become `HistoryEntry{wql, namespace, elapsed_ms, rows, at}`; migrate from `Vec<String>`. *AC: an old config's plain strings load with `None` metadata.* Verified against the **real** `config.json` on this machine: two bare strings became objects with `null` metadata while the new run recorded `elapsed_ms: 79, rows: 1`. Both render side by side in the capture, the migrated ones as em dashes. History dedupes on (wql, namespace), not on wql alone.
- [x] **4.7** `[gui]` Debounce `Config::save()` — today `push_history` writes `config.json` to disk on **every** query run. *AC: 10 rapid queries produce ≤ 2 writes.* Leading-edge cooldown rather than a trailing-edge delay, so an endless stream of queries cannot starve the write; unit-tested at exactly 2 for a ten-run burst.
- [x] **4.8** `[gui]` `views/events.rs` left column (300px): event-query textarea, `WITHIN` interval field, Delivery segmented (Permanent **disabled** + tooltip), Start/Stop + Clear. *AC: the WITHIN value is injected into the WQL.* Met, and verified through WMI rather than through the string: injecting 9 into the default query rewrote it to `WITHIN 9` **and** the subscription then delivered seven events on one stamp (`09:25:18.185`) where `WITHIN 2` had delivered two batches two seconds apart. The field is **two-way** — it writes the clause, and re-reads it whenever it is not focused, because `views/explorer/detail.rs`'s "Watch" action writes a whole query and a stale interval box would be a lie about the running subscription. It is **disabled** with a reason on a query that is not intrinsic (`WITHIN` is not invented for `Win32_ProcessStartTrace`). The Delivery control is hand-built in the view: `widgets::button::segmented` has no disabled state and `widgets/` was out of scope, so Permanent is an `add_enabled(false, ..)` with `on_disabled_hover_text` naming the filter/consumer/binding it would write and the three things (typed confirmation, audit line, teardown) that are not built.
- [x] **4.9** `[gui]` Events stats card: Events received, Delivery rate (events/s over a 30s window), ~~Provider~~, **Queued** (real mpsc backlog — "Sink queue" is not observable). *AC: rate is computed, never constant.* Rate measured at **0.51, 1.54, 240.02, 261.13 / s** across four runs of two different subscriptions; it is an em dash before one second of observation, never a zero standing in for "not measured". **"Provider" was dropped**, for the same reason "Sink queue" was: nothing in a notification reply names the provider, and reading `__Win32Provider` would mean a new core request. Its slot went to **Delivery** — `polled / N s` or `pushed`, derived from the query's event class, which *is* something we hold. Queued reports the depth of our own channel at the last drain **and its peak since Start**, because an instantaneous zero hides a backlog that has already cleared (peak 111 was observed while the instantaneous read was 19). A fifth row, **Dropped (cap)**, appears only once the ring has evicted something.
- [x] **4.10** `[gui]` Events stream header: pulsing live dot + "N events since HH:MM:SS" + Filter + Save log. *AC: the pulse is driven by `input().time`, not `animate_bool` (which cannot loop).* Met and measured across 18 successive `PrintWindow` grabs ~60 ms apart: the dot's mean luminance ran 255 → 262 → 120 → 129, i.e. a full descent and the start of the next rise. `animate_bool` could not have produced it (see `docs/FINDINGS.md`). **The clock is UTC and says so**: the app has no local-time conversion anywhere, and a UTC reading drawn as if it were local is silently wrong for the one job an event log has. Filter is the kit's `filter_box` rather than the mock's Filter *button* — every other list in the app filters that way.
- [x] **4.11** `[gui]` Event row: timestamp | kind badge (Creation=ok / Modification=accent / Deletion=bad / Operation=muted) | class | detail | open icon. ~~*AC: kind is parsed from `__CLASS` of the event.*~~ **The AC is impossible as written and was replaced.** `list_properties` calls `GetNames` with `WBEM_FLAG_NONSYSTEM_ONLY`, so `__CLASS` is filtered out before `flatten_event` sees it; measured, the delivered key set of a process-creation event is exactly `SECURITY_DESCRIPTOR, TIME_CREATED, TargetInstance.{Caption,Name,ParentProcessId,ProcessId}`. The target class is missing too — `flatten_event` copies six named scalars out of the embedded object and none is a class name. Kind and class are therefore **derived from the subscription's query and stamped on the row at arrival**, with the one surviving per-event discriminator (`PreviousInstance`, which only a modification carries) used to narrow an `__InstanceOperationEvent` subscription as far as it honestly goes. Full detail in `docs/FINDINGS.md`. The open icon opens a right-hand reveal listing every pair as delivered — which is also the screen that proves the above.
- [x] **4.12** `[gui]` New-row accent flash-in: per-row `created_at` from `input().time` + `easing::cubic_out((age/0.18).clamp(0,1))`. **Not** `animate_bool`, which returns the target on the first frame for a fresh Id and would never fade in. *AC: a new row visibly flashes accent once.* Met; caught in three consecutive grabs, the row band running `(50.3, 61.4, 78.1)` → `(44.7, 53.1, 68.8)` → `(35.9, 38.0, 51.4)` → rest at `(35.1, 37.2, 50.7)`. **The mock's second half was dropped after measuring it**: `noct-row` also ramps the row's *content* from zero opacity, and an intrinsic subscription delivers a whole poll on one frame — so at 260 ev/s every visible row shares a birth time and the ramp blanked the entire stream for 0.18 s (captured, then removed). The accent wash alone says "new"; text stays legible over an 18 % tint.
- [x] **4.13** `[gui]` Events log capped ring buffer (replacing the 500-element `Vec` front-insert + truncate). *AC: sustained 200 ev/s does not degrade frame time.* Met at **240–261 ev/s sustained** against a real subscription, 8 830 events over ~34 s with the log full: `App::ui` cost **p50 0.63 ms, p95 1.12 ms, p99 1.34 ms**, worst burst frame 2.58 ms. ~~But the task's premise is wrong about *what* degrades frame time.~~ Measured both halves: the front-insert at the old 500-row cap costs 0.78 µs/event against the ring's 0.41 — 1.9×, i.e. 156 µs per second at 200 ev/s, which is nothing. What the ring actually buys is a cap that can be *raised*: at 5 000 rows the front-insert is 4.60 µs/event (11.4×) because its cost is linear in the cap. The frame-time problem was the un-virtualised list next to it — the old view laid out every retained row every frame, and the same stream through `ScrollArea::show` instead of `show_rows` costs **46.3 ms mean and 54.5 ms p99 at 5 000 rows**, three times over budget. Cap is 5 000 (the Process view's figure), and eviction is counted and shown rather than silent. Numbers and method in `docs/FINDINGS.md`.
- [x] **4.14** `[gui]` `views/saved.rs`: card grid — star/favourite, title, folder tag, mono query preview, meta (ms · rows · author), New query. *AC: cards render at `minmax(310px)`.* Captured, 3 columns at 1240px. **`widgets::card::card_grid` hands its item closure a *horizontal* `Ui`** (`allocate_ui` inherits the layout of the enclosing `horizontal_top`), so a card body must open its own `ui.vertical`. Without it the four bands render side by side and the mono preview wraps to one character per line — captured before the fix. `views/explorer/methods.rs` calls `card_grid` the same way and looks like it has the same bug; **not captured, not fixed here** (not this task's files).
- [x] **4.15** `[gui]` `SavedQuery` v2: `+folder, +fav, +author, +last_ms, +last_rows`; author = `USERNAME`/`USERDOMAIN` at save time. *AC: old saved queries migrate with empty folder and `fav=false`.* Author captured as the real `DESKTOP-6SAB9EN\root`. `last_ms`/`last_rows` are stamped by `Config::note_query_run` when a reply for that exact query in that exact namespace lands, and render as an em dash until then — never a zero. **Gap in the plan:** nothing in Phase 4 *sets* a folder, which would have left the field decorative; `Config::set_folder` plus a click-to-edit folder tag on the card were added to close it.
- [x] **4.16** `[gui]` **Fix:** applying a saved query restores `namespace` **and** `wql` ~~(today `app.rs:1142-1145` restores only `wql`…)~~. *AC: a query saved under `root\subscription` reopens there.* **The cited call site no longer exists** — the Phase 3 Explorer rebuild deleted the saved-query list along with `views/explorer/central.rs`, so between then and now there was no way to apply a saved query at all. The fix is therefore built-in rather than repaired: `apply_saved_query` calls `select_namespace` **before** setting the text and running, because `run_query` reads `active_ns`. The card carries its namespace as an outline tag so the difference is visible before the click.
- [x] **4.17** `[gui]` Saved library Import…/Export… via `rfd` (a UNC path is a valid file path). Header reads `Local library · N queries · <path>` — **no "synced" claim**. *AC: export→import round-trips losslessly.* Round trip unit-tested (export → parse → merge into an empty library reproduces it exactly). The path shown is `config.json` itself, because that file *is* the library. The Import tooltip names UNC explicitly and says the file is read once, not synchronised.
- [x] **4.18** `[gui]` Folder filter + favourites filter on the Saved view. *AC: both filter live.* Chips rather than a combo. `Some("")` is the ungrouped bucket and is a real selection, distinct from "no filter". Favourites-only captured showing 1 of 4 cards; folder/text/favourite composition is unit-tested.
- [ ] **4.19** `[infra]` Release v0.10.0.

### Phase 5 — Machines + multi-host (v0.11.0)

- [x] **5.1** `[core]` Add `host: Option<String>` to **every** `Response` variant, stamped at *execution* time — read on the worker thread after every earlier request in the queue has run and before this one does. *AC: a `SetHost` mid-flight can no longer mislabel a result.* Measured (`examples/hosts.rs`): queue `Query`, `SetHost(\\HOST)`, `Query` together, and the replies come back stamped `None`, `Some("HOST")`, `Some("HOST")` — while the host in force when all three are *read* is `HOST` for all three.
- [x] **5.2** `[core]` Add `namespace` to `Instances`, `MethodDone`, `Mof`, `Providers`, `Network`, `EventSubscriptions`. *AC: no response requires out-of-band namespace tracking.* Note: `Network` echoes `root\StandardCimv2` (where the rows come from; the process names are joined from `root\CIMV2` on the same host) and `EventSubscriptions` echoes `root\subscription`, the first of `SUBSCRIPTION_NAMESPACES` — that scan spans two namespaces and no single string can name both. Per-row namespace provenance was **not** added.
- [x] **5.3** `[core]` `HostRef` + `WorkerRegistry: HashMap<HostRef, WmiWorker>` — one COM thread per host; ~~`Request` gains a `target: HostRef`~~ **the target is an argument to `WorkerRegistry::send`, not a field on every `Request` variant.** Routing is envelope, not payload: putting it in the payload would have meant a `target` on all 17 variants and a change at every construction site in the GUI, to say something none of the request handlers read. `poll()` returns `(HostRef, Response)`. *AC: two hosts can be queried without a `SetHost` flush between them* — measured with `HostRef::Local` and `HostRef::Sso{host}` for this machine's own name, four interleaved queries, correct stamps, no reconnects.
- [x] **5.4** `[core]` `Response::HostConnected{id, host, connect_ms, probe_ms, info: HostInfo}`; the probe result is no longer discarded. *AC: connect returns a real bind time.* Measured: local 24 ms connect + 36 ms probe, same machine by name 25 ms + 32 ms.
- [x] **5.5** `[core]` `HostInfo` from `SELECT Caption, Version, BuildNumber, OSArchitecture, LastBootUpTime FROM Win32_OperatingSystem` + `SELECT UUID FROM Win32_ComputerSystemProduct`. *AC: build number and machine UUID populate.* Both do. The UUID query is allowed to fail without failing the connect — it is a different provider, and a host that will not answer it is still connected.
- [x] **5.6** `[core]` **Bug fix:** `list_connections` routes through the credential dispatcher instead of calling `connect()` directly. *AC: under alt-creds the `process` column is populated, not blank.* The AC itself needs a live remote and is **unverified**. What was measured is the split that caused it: on the pre-fix code, with the worker in alt-cred mode and credentials that cannot connect, `NetworkSnapshot` still returned `Ok` — the `Win32_Process` half succeeded over SSO while both endpoint queries failed on the credentialed transport, and the swallowed errors turned that into an empty-but-successful snapshot. Two halves of one function, two transports.
- [x] **5.7** `[core]` `RemoteConn::exec_objects` returning `IWbemClassObject`s, cap/deadline/cancel-bounded like every other enumeration. Also `Bound::exec_objects`, so both transports reach it the same way. *AC: a reflective walk is possible over alt-creds.*
- [x] **5.8** `[core]` **Bug fix:** the `if !is_alt_cred()` guard on the `__EventConsumer` walk is gone, and the walk reads objects (5.7) rather than flattened maps on **both** transports, because `__CLASS` is a system property that the enumeration flags hide. *AC: a credentialed remote subscription scan returns non-empty `consumer_type`/`action`* — needs a live remote, **unverified**. Verified instead: the local scan produces byte-identical output through the new shared path, and a unit test pins the consequence the guard had (`assess("", benign_filter, "")` is `Low` for a subscription that is `High` once its consumer is read).
- [x] **5.9** `[core]` `read_class_schema`, `class_mof`, `list_instances`, `invoke_method`, `build_search_index` and the class enumeration all go through the dispatcher. The `wmi` crate's `WMIConnection` is gone from the worker entirely — `read_class_schema`/`object_mof` take an `IWbemClassObject`, method invocation is raw `GetMethod`/`SpawnInstance`/`Put`/`ExecMethod`. There is now exactly one way to bind a namespace, which is the actual fix: the bug was never a single call site, it was having a second door. *AC: schema loads over alt-creds* — **unverified** (live remote). Measured: with unreachable alt-creds, all 15 request shapes refuse; before, 7 answered from a current-user connection, including a method that really executed locally.
- [x] **5.10** `[core]` `CoSetProxyBlanket` impersonation level parameterised (`IDENTIFY`/`IMPERSONATE`/`DELEGATE`). *AC: the setting changes the blanket call.* ~~on the alt-cred path~~ — **on both paths, and the plan's stated limit is wrong.** `enumerate::DirectConn` hand-calls `CoSetProxyBlanket` too (it has since Phase 3; the SSO path stopped going through the `wmi` crate then), so the level is reachable there and *is* honoured: measured locally at `Identify`, the connect probe and an ordinary query are refused with `WBEM_E_ACCESS_DENIED`, `StdRegProv.EnumKey` with `WBEM_E_PROVIDER_NOT_CAPABLE`, while reflecting a class schema still succeeds — the level gates providers, not the repository. What could **not** be tested is the alt-cred path's use of it: `ConnectServer` rejects a credentialed local connection with `WBEM_E_LOCAL_CREDENTIALS` *before* the blanket call, so the three levels are indistinguishable there on one machine.
- [x] **5.11** `[core]` `Msft_Providers` query widened with ~~`HostingModel`~~, `HostingSpecification`, `user`, ~~`Result`~~. *AC: new columns available* — **met, but two of the four columns are not on that class.** Measured: `SELECT HostingModel FROM Msft_Providers` and `SELECT Result FROM Msft_Providers` are both *Invalid query* on Windows 11 26200, and the class declares neither. `HostingModel` is real on `__Win32Provider` in each provider's *own* namespace and is now joined from there by `Name` (`NetworkServiceHost`, `LocalSystemHost`, `Decoupled:NonCOM`, `WmiCore` on this machine; empty for `DelegatorProvider`, which has no registration — a real state, not a failure). `Result` has no queryable home at all: the nearest thing is `ResultCode` on the `Msft_WmiProvider_*_Post` *extrinsic event* classes, so it was **left out** rather than added as a permanently empty field. `HostingSpecification` is passed through undecoded — it tracks `HostingModel` one-for-one across the eight rows here, which is an observation, not a mapping table. See `docs/FINDINGS.md`.
- [x] **5.12** `[core]` Provider host stats: `SELECT Name, IDProcess, PercentProcessorTime, PrivateBytes, WorkingSetPrivate, HandleCount, ThreadCount FROM Win32_PerfFormattedData_PerfProc_Process` filtered by the host PIDs the provider list actually names, ~~`WHERE Name LIKE 'WmiPrvSE%'`~~, joined on `IDProcess` (**never** on `Name`). *AC: sibling hosts `WmiPrvSE#1/#2` are distinguished* — **met.** The plan's name filter is **wrong twice over.** It misses hosts: two of the eight providers here live in the WMI service itself (PID 2788, counter instance `svchost#13`), which `LIKE 'WmiPrvSE%'` never matches. And it buys nothing: measured, a five-PID `WHERE` takes 347–476 ms against 380–388 ms for the *unfiltered* 393-row enumeration, because the perf provider builds every instance either way. `PrivateBytes` was added — it is the private-commit reading the memory quota is compared against, and `WorkingSetPrivate` alone is 25–40 % lower. Also measured: `PercentProcessorTime` is summed over all 24 logical CPUs (`_Total` reads 2414), so the raw counter is **not** a percentage and `NumberOfLogicalProcessors` is read alongside it.
- [x] **5.13** `[core]` `__ProviderHostQuotaConfiguration` (namespace `root`) → `MemoryPerHost`, `HandlesPerHost`, `ThreadsPerHost`, plus `MemoryAllHosts` and `ProcessLimitAllHosts` from the same singleton. *AC: quota ceilings available for the usage bars* — **met.** Read here as 512 MB / 4096 / 256 / 1024 MB / 32. `HostQuota::pressure` reports the **worst** of the three fractions, not a blend: a host is killed by whichever ceiling it reaches first. A zero ceiling means *unlimited* and yields `None`, never `0 %`. **Unverified:** which counter WMI actually enforces `MemoryPerHost` against — `PrivateBytes` is the closest reading available (measured equal to `Win32_Process.PrivatePageCount` on the same PID) but confirming it would mean lowering the quota until a host is really killed.
...
- [x] **6.5** `[gui]` `views/compare.rs`: A/B host pickers (from `Config.targets`) + class/WQL picker + Run. *AC: A and B run against different `HostRef`s without a `SetHost` race* — **met**, and the race is removed by construction rather than by ordering: the view owns a `WorkerRegistry`, so each side is a COM thread of its own and neither ever issues a `SetHost` the other could observe. One limitation is stated on screen rather than worked around: an alternate-credential target is listed **disabled**, because no password is persisted and Compare has no prompt for one, so it could only ever open a connection that fails.
- [x] **6.6** `[gui]` Diff legend (Identical / Changed / Only on A / Only on B) + Export diff (JSON + CSV). *AC: export round-trips* — **met.** The JSON is the `TableDiff` plus the context that makes it readable later (both hosts, the query **that ran**, the key and its source, the ignore list, and whether either side was whole); the CSV is one line per *side-row*, so a changed row is two lines and every cell holds one value. Measured on a live local A/B of `Win32_Process`: 383 CSV rows over 50 columns, `349 =`, `17 !=` × 2 sides, and the identical rows recovered from B's order because `TableDiff` counts them rather than carrying them.
- [x] **6.7** `[gui]` Diff table: sign column (~~`=` `≠` `−` `+`~~ → `=` `!=` `-` `+`) + per-cell tinted backgrounds. Value columns are **unclipped** so the tint is not discarded (`clip_rect_margin == 0.0`). *AC: tints are visible on every changed cell* — **met** (capture: a changed `PageFaults` cell renders `10788122 → 10788128` on a warn tint while its unchanged neighbours are flat). **The two typographic signs are struck through**: `check.ps1`'s I9 allow-list does not carry U+2260 or U+2212 and fails on them. The plan's implied reason — that a glyph outside the list is missing from the embedded faces — is **not** why here: both codepoints are in the cmap of `InterVariable.ttf` *and* `JetBrainsMonoNL-Regular.ttf`, read out of the two files. Widening I9 is a decision about the invariant, not one a view may take, so the sign column uses the ASCII pair a diff reads as anyway.
- [x] **6.8** `[gui]` Auto-key from `key_columns` with a manual override combo; ignore-columns multiselect defaulting to volatile columns (PID, timestamps, counters). *AC: `Win32_Service` diffs on `Name`, not whole-row equality* — **met** (on-screen: `key: Name (class key)`, `ignoring: 3` = `InstallDate, ProcessId, __PATH`). Two things the plan does not mention and both of which are wrong answers given confidently without them. **A declared key that the query did not return is not a key**: `SELECT State FROM Win32_Service` still reports `Name`, and keying on an absent column gives every row the same empty key — so an unusable key falls back to `__RELPATH` and then to whole-row, and says which on screen. **A key column is never auto-ignored**: `Win32_Process.Handle` is the PID *and* the class key, and `diff_tables` builds its whole-row fallback key out of the non-ignored columns, so ignoring it can quietly remove it from the identity.
- [x] **6.9** `[gui]` Compare empty/partial states (one side failed, one side unauthorized, differing schemas). *AC: a failed B side shows an error card, not an empty table* — **met** (capture: A returns 309 rows while B carries `The RPC server is unavailable. (0x800706BA)` on the side card, in a banner, and in the status bar). The state the plan does not name is the dangerous one: **a truncated side**. Every row it never read would be reported as a difference the two machines do not have, so the table is withheld with both completions spelled out, and the one way past it is an explicit *Diff the partial results anyway* — after which the comparison is labelled partial on screen, in the status bar, and as `"sound": false` in the export. Differing column sets get a banner naming the columns and a one-click *Compare shared columns only*.
- [ ] **6.10** `[infra]` Release v0.12.0.

### Phase 7 — Polish + 1.0 (v1.0.0)

- [x] **7.1** `[gui]` Settings → Connection: default namespace, impersonation level, authentication, operation timeout — all wired to real behavior. *AC: no setting is decorative* — **met, with one exception that is stated on screen.** Default namespace and operation timeout were wired; the impersonation row's tooltip claimed it was "honoured only once core task 5.10 parameterises `CoSetProxyBlanket`", which is ~~true~~ **stale** — `worker.rs` sets its own blanket on *both* transports and `Request::SetHost` has carried the level since Phase 5, so the control was disabled over plumbing that already existed. **Authentication stays disabled**, and the plan is wrong to have listed it as wireable: the package is not a variable. `remote.rs:131` pins the alternate-credential path to `RPC_C_AUTHN_WINNT`, and the SSO path goes through the `wmi` crate, which negotiates and exposes no hook — so there is nothing to choose between without a core change. It now *reports* which package the live connection is using rather than showing a dead default.
- [x] **7.2** `[gui]` Settings → Results: row limit (→ `Request::Query.max_rows`), live polling toggle, byte formatting, show system classes. *AC: row limit visibly truncates* — **met.** Row limit and timeout were already reaching `Request::Query` (`state::requests::run_query`); only the controls were disabled. Byte formatting was already consumed by the Providers quota meters. `show_system_classes` had no consumer at all and now filters the Explorer class list — as a *list* filter, never a data one: the System chip still lists every hidden class, and the list states how many it is holding back. The predicate is the name (`__` prefix), not `ClassKind::SYSTEM`, because the row's label promises "names beginning `__`" and the qualifier flag is a different, larger set.
- [x] **7.3** `[gui]` Settings → Code generation: default language, include-credentials block, line width. *AC: generated scripts respect all three* — **two of three met, and the third could not be.** The credentials block rewrites the line that *binds* WMI in each language (PowerShell gains a `CimSession`, VBScript swaps the `winmgmts:` moniker — which takes no credential at all — for `SWbemLocator.ConnectServer`, C# gains `ConnectionOptions`), because a declared-then-ignored credential header reads as working. ~~Line width wraps generated scripts.~~ **There is no column at which a generated script can be wrapped**: the PowerShell arm carries its query in a here-string, the C# arm in a verbatim literal and VBScript's in a plain literal, so a newline inserted to fit a width is *part of the string* and changes the query the script runs. The setting drives the Code sub-tab's column guide instead, and the row says so rather than keeping a label it cannot honour.
- [x] **7.4** `[gui]` Settings → About/Licences panel rendering the bundled OFL + MIT texts. *AC: satisfies the OFL bundling clause* — **met** (capture: the Inter OFL rendered inline under Settings → About). `include_str!`, not a runtime read: the obligation is to distribute the text *with* the software, and a panel that read a file from beside the executable would show nothing once the binary was copied on its own.
- [x] **7.5** `[gui]` Keyboard map overlay (`?` or `F1`) listing every shortcut. *AC: matches the actual `KeyboardShortcut` registrations* — **met by construction rather than by matching.** The four `const KeyboardShortcut`s became one `BINDINGS` table carrying the chord, the action, the scope and the description; `handle_shortcuts` dispatches from it and the overlay renders it, so there is nothing for the two to disagree about. F1 is itself a row in the table. The palette's Up/Down/Enter/Esc are listed as a second, separately-headed group because they are **not** `KeyboardShortcut`s and cannot be — the palette consumes them off the event queue before its `TextEdit` can claim them.
- [x] **7.6** `[gui]` Empty/loading/error state audit across all 10 views. *AC: a checklist of 30 states, all covered* — **met**; the audit found three real gaps rather than thirty missing states. The Network table and the Explorer class list each drew a bare header rule over an empty rectangle, in situations that mean different things (no snapshot yet / nothing there / the filters exclude everything), and each now says which. The status bar's catch-all arm called **Saved, Machines and Settings "not built yet"** — true when eight of eleven destinations had views, false for the last three phases; that match is now exhaustive by name, so a destination added without a status line is a compile error. `empty_state` moved into `widgets::loading` after the third copy of it appeared.
- [x] **7.7** `[gui]` Focus-ring audit: ~~every interactive widget in the kit and every view calls `focus_ring`~~ → **the ring is painted once a frame for whatever holds focus.** *AC: full keyboard traversal shows a ring at every stop* — met, and the plan's mechanism is what the audit disproved. Asking every call site to remember had already failed eleven times: three `ui.checkbox`, eight `ui.selectable_label`, plus `CollapsingHeader` — every raw egui widget a view reaches for is one the kit never sees, and no amount of auditing keeps that list at zero. `Context::read_response(Memory::focused())` makes it structural instead. Verified by capture with focus forced onto Network's "external only" checkbox, a control that never called `focus_ring` and never will.
- [x] **7.8** `[gui]` Move the remaining blocking UI-thread work off the frame. *AC: opening a file dialog does not stall the frame loop* — **met.** One IO thread, one job queue, drained once a frame. `util::save_file` keeps its two-argument signature, so all fifteen call sites — including the ones in view modules this pass did not own — became non-blocking without being touched. Two things the plan does not mention and both are consequences worth stating: a dialog opened off the UI thread is **not owned by the window**, so it is a separate top-level window and the app stays interactive behind it; and the `let _ = fs::write(..)` that used to swallow a failed export now reports it. `push_history` was already debounced by task 4.7, so the remaining blocking write was `Config::save` itself.
- [x] **7.9** `[gui]` `ViewportCommand::Close` gate: if a method invocation is in flight, answer with `CancelClose` and confirm. *AC: closing mid-invoke prompts* — **met** (capture: `Win32_Process.Create · 7.8s so far`, Wait / Close anyway). eframe 0.35 has no `on_close_event`; a close arrives as `ViewportInfo::close_requested()` for one frame and must be refused in that same frame, which is why the gate runs before anything in `ui` that could return early. The dialog does not offer to cancel the call, because it cannot: the confirmation says the call is already at WMI, will still run, and that its result is what is lost.
- [x] **7.10** `[infra]` Perf benchmark example: 50k-row table, 1,400-class list, 2k-event stream. ~~*AC: all three hold 60 fps.*~~ → **measured, release build, this machine.** 240 frames a scenario, first 60 discarded: **50k-row table 0.89 ms mean / 1.24 ms p95**, **1,400-class list 0.58 / 0.76**, **2,000-event stream 1.19 / 1.50**, against a 16.67 ms budget. Three corrections to how the plan states the AC. (a) "Holds 60 fps" is not observable from the frame interval, which sat at **13.3 ms** in all three — that is the display period, and it would read the same for code ten times slower; only `eframe`'s `cpu_usage` says anything. (b) It is a **mode of the binary** (`--bench`), not an `examples/` target: every widget it must exercise is `pub(crate)`, so an example could only measure a reimplementation of the table rather than the table. (c) The third scenario drives the **Process** view, not Events: `EventLog::push` and `EventRow` are private to `views::events`, so a synthetic stream cannot be installed there from outside it. Process is the same shape and strictly heavier — every row also computes a fade alpha, a lifetime and a filter match per frame.
- [x] **7.11** `[infra]` Decide and document `egui_extras` `serde` feature (column widths currently use `get_temp` and are lost on restart). *AC: decision recorded in CHANGELOG* — **OFF**, recorded. The plan's premise is right and its implied fix does not work: `egui_extras` does switch `get_temp` → `get_persisted` under the feature, but **`eframe/persistence` is off** (`ron` is not in `Cargo.lock`), so egui's memory is never written to disk and the widths are lost either way. Reaching the benefit means turning on `eframe/persistence`, which puts a second preferences file beside `config.json` with no version field and no migration, against a `Config` that has both. Column widths are keyed by `id_salt`, so a persisted width also outlives the column set it was measured for.
- [ ] **7.12** `[infra]` README rewrite + new screenshots + CHANGELOG for 1.0. *AC: every screenshot is the Nocturne shell.*
- [x] **7.13** `[infra]` `cargo deny check` against the new deps (winit, fonts are data not deps). *AC: `deny.toml` passes* — **met**, after two decisions rather than a clean first run. `BSL-1.0` (Boost) is added to the allow-list: OSI-approved, permissive, and its notice clause explicitly does not apply to object code — it arrives via `clipboard-win` and `error-code` under `arboard`, i.e. the Windows clipboard, so it is not an optional corner. `RUSTSEC-2026-0192` (`ttf-parser` unmaintained) is ignored with the reason written into `deny.toml`: it is a maintenance notice rather than a vulnerability, its own text says no upgrade exists, and its only path here is `sctk-adwaita` — winit's **Wayland client-side decorations**, which never compile on Windows and are not the font path this tool uses (egui rasterises with `skrifa`, the crate the advisory recommends). Dropping it would mean dropping eframe's `wayland` feature and regressing Linux to X11-only to silence a notice about a crate we never call.
